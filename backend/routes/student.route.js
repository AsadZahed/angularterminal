...
//terminal
let EmployeeLeaveSchema = require('../model/employeeLeaveSchema');

// Get employee leave
studentRoute.route('/viewEmployeeLeaves').get((req, res) => {
  EmployeeLeaveSchema.find((error, data) => {
    if (error) {
      return next(error)
    } else {
      res.json(data)
...
// Add Employee Leaves
studentRoute.route('/addEmployeeLeaves').post((req, res, next) => {
  EmployeeLeaveSchema.create(req.body, (error, data) => {
    if (error) {
      return next(error)
    } else {
      res.json(data)
